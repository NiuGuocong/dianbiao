...
public class DateUtil {
    public static Object dateToString;
    ...
    public static long diffMinutes(Date bigDate, Date smallDate) {
        if (bigDate == null || smallDate == null) {
            return 0;
        }
        return (bigDate.getTime() - smallDate.getTime()) / (60000);
    }

    /**
     * 计算两个日期之间相差的天数
     *
     * @param beganDate 较小的时间
     * @param endDate   较大的时间
     * @return 相差天数
     * @throws ParseException
     */
    public static int getDaysBetween(Date beganDate, Date endDate) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        beganDate = sdf.parse(sdf.format(beganDate));
        endDate = sdf.parse(sdf.format(endDate));
        Calendar cal = Calendar.getInstance();
        cal.setTime(beganDate);
     ...
     * 字符串的日期格式的计算
     */
    public static int getDaysBetween(String beganDate, String endDate) throws ParseException {
        SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
        Calendar cal = Calendar.getInstance();
        cal.setTime(sdf.parse(beganDate));
        long time1 = cal.getTimeInMillis();
...
//    	}
        Date startDate = DateUtil.stringToDate("2013-12-10 17:15:20", "yyyy-MM-dd HH:mm:ss");
        Date endDate = new Date();
        differenceMinute(startDate, endDate);
        Logger.info("args = " + DateUtil.lastMonthOfEndDayByDate(new Date()));


        Date date1 = DateUtil.stringToDate("2013-12-10 17:00:20", "yyyy-MM-dd HH:mm:ss");
        Date date2 = DateUtil.stringToDate("2013-12-10 17:50:00", "yyyy-MM-dd HH:mm:ss");
        System.out.println(startDate + " - " + date1 + "=" + diffMinutes(startDate, date1));
        System.out.println("s-2=" + diffMinutes(startDate, date2));
